...
export function Sidebar({ children, side = "left", variant = "sidebar", collapsible = "offcanvas" }) {
  const { isMobile, openMobile, setOpenMobile } = React.useContext(SidebarContext);

  if (collapsible === "none") {
    return <div className="flex h-full w-64 flex-col bg-sidebar text-sidebar-foreground">{children}</div>;
  }
...
const Sidebar = React.forwardRef(({ state, collapsible, variant, side, className, children, ...props }, ref) => {
  return (
    <div
      ref={ref}
...
const SidebarMenuAction = React.forwardRef(({ showOnHover, className, ...props }, ref) => {
  const Comp = "button";
  return (
    <Comp
      ref={ref}
...
const SidebarMenuSkeleton = React.forwardRef(({ className, showIcon = false, ...props }, ref) => {
  ...
  return (
    <div ref={ref} data-sidebar="menu-skeleton" className={cn("rounded-md h-8 flex gap-2 px-2 items-center", className)} {...props}>
      {showIcon && <div className="size-4 rounded-md bg-gray-300" data-sidebar="menu-skeleton-icon" />}
      <div
        className="h-4 flex-1 max-w-[--skeleton-width] bg-gray-300"
